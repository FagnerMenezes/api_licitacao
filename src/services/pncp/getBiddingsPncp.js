...
async function getBiddingsNoticesPNCP(
	...
	try {
		 ...
		 * @type {number} total
		 */
		const total = Number.parseInt(response.total) ?? 0;
...
async function getDataBiddingsPNCP(cnpj, code, year) {
	try {
		console.log(cnpj, code, year);
		//const url = `https://pncp.gov.br/api/pncp/v1/orgaos/${cnpj}/compras/${year}/${code}`;
		const codePncp = code;
		let verificador = "";
		if (codePncp.length >= 5) {
			verificador = "0";
		} else if (codePncp.length >= 6) {
			verificador = "";
		} else if (codePncp.length >= 4) {
			verificador = "00";
		} else if (codePncp.length >= 3) {
			verificador = "000";
		} else {
			verificador = "00";
		}
		console.log(verificador + code, "verificador");
		const code_pncp = `${cnpj}-1-${verificador}${code}/${year}`;
		console.log(code_pncp);
		const url = `https://pncp.gov.br/api/search/?q=${code_pncp}&tipos_documento=edital&ordenacao=-data&pagina=1&tam_pagina=10&status=recebendo_proposta`;
		const dataBidding = await axios
			.get(url)
			.then(async (element) => {
				if (element?.data?.items[0]?.data_fim_vigencia === undefined) return;
				const data = {
					process_data: {
						status: "Cadastrar proposta",
						type_dispute: "Menor preço unitário",
						modality:
							element?.data?.items[0]?.modalidade_licitacao_id === 8
								? "DL"
								: element?.data?.items[0]?.modalidade_licitacao_id === 6
									? "PE"
									: "PE",
						portal:
							await checkPortalPncp(
								"Compras.gov.br",
							) /**element?.data?.items[0]?.data?.usuarioNome */,

						n_process: String(element?.data?.items[0]?.title).replace(
							/[^0-9]/gi,
							"",
						),
						bidding_notice: String(element?.data?.items[0]?.title).replace(
							/[^0-9]/gi,
							"",
						) /** `${element?.data?.numeroCompra}${element.data?.anoCompra}`*/,
						date_finish: String(
							element?.data?.items[0]?.data_fim_vigencia,
						)?.slice(
							0,
							10,
						) /**String(element?.data?.dataEncerramentoProposta)?.slice(
							0,
							10,
						),*/,
						date_init: String(
							element?.data?.items[0]?.data_inicio_vigencia,
						)?.slice(
							0,
							10,
						) /**String(element.data?.dataAberturaProposta)?.slice(0, 10),*/,
						hours_finish: String(
							element?.data?.items[0]?.data_fim_vigencia,
						)?.slice(11, 16) /**String(
							element?.data?.dataEncerramentoProposta,
						)?.slice(11, 16),*/,
						object:
							element?.data?.items[0]
								?.description /**  element?.data.objetoCompra,*/,
						code_pncp:
							element?.data?.items[0]
								?.numero_controle_pncp /**element?.data?.numeroControlePNCP,*/,
						srp: "" /**?.data?.srp,*/,
						code_bidding_pcp: "",
					},
					government: [
						{
							_id: ID(),
							name: `${element.data?.items[0]?.orgao_nome} - ${element.data?.items[0]?.unidade_nome} - ${element.data?.items[0]?.uf}` /** `${element.data?.orgaoEntidade?.razaoSocial} - ${element.data?.unidadeOrgao?.nomeUnidade} - ${element.data?.unidadeOrgao?.ufSigla}`,*/,
							cnpj: element.data?.items[0]
								?.orgao_cnpj /**element.data?.orgaoEntidade?.cnpj,*/,
							code_government:
								element?.data?.items[0]
									?.unidade_codigo /**element?.data?.unidadeOrgao?.codigoUnidade,*/,
							manager: "true",
							address: [
								{
									id: ID(),
									...
									district: "",
									zip_code: "",
									uf: element.data?.items[0]
										?.uf /**element?.data?.unidadeOrgao?.ufSigla,*/,
									city: element?.data?.items[0]
										?.municipio_nome /**?.data?.unidadeOrgao?.municipioNome,*/,
									complement: "",
								},
							],
							contact: [
			...
			.catch((error) => {
				console.log(error, "getDataBiddingsPNCP");
		...
		if (data.total > 0) {
			...
			const dataSet = dataAllBidding.map((response) => {
				...
				if (filteredItems.length > 0) {
					...
					// console.log(
					// 	response?.biddingInfo?.government,
					// 	response?.biddingInfo?.process_data,
					// );
					return {
						_id: ID(),
...
const getCaptcha = async () => {
	try {
		const url =
			"https://api.hcaptcha.com/checksiteconfig?v=94cdacf&host=pncp.gov.br&sitekey=57e81b38-5fda-4cac-8a22-d39824287a00&sc=1&swa=1&spst=1";
		const response = await axios.get(url);
		const captcha = response.data.c.req;
		console.log(captcha);
		return captcha;
	} catch (error) {
		console.log(error.message, "getCaptcha");
		return {};
	}
};
