...
const getDataBiddingsComprasnet = async (
	...
	try {
		...
		const result = biddingPromise.flat();
...
async function totalBiddings(dt_inicio, dt_fim) {
	...
	const data = await axios.get(url).then((html) => {
		...
		const total = Number.parseInt(totalBiddings);
